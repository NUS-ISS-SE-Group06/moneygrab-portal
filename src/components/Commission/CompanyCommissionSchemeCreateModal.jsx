...
useEffect(() => {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await api.post(`/api/v1/company-commission-schemes`, {
        moneyChangerId: companyCommissionScheme?.moneyChangerId,
        schemeId: companyCommissionScheme?.schemeId,
        createdBy: userId,
      });

      const created = response.data;
      console.log("NEW-Response from server:", created);
      const enrinched = {
        ...created,
        companyName: companyCommissionScheme?.companyName,
        nameTag: companyCommissionScheme?.nameTag,
      };
...
CompanyCommissionSchemeCreateModal.propTypes = {
  selectedScheme: PropTypes.object.isRequired, 
  onClose: PropTypes.func.isRequired,
  onCreated: PropTypes.func.isRequired
};

export default CompanyCommissionSchemeCreateModal;